...
class Listing(Base):
    ...
    first_published_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
...
class Application(Base):
    ...
    status_reason_code: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    outcome_code: Mapped[Optional[str]] = mapped_column(String(50), nullable=True, index=True)
    outcome_reason_code: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow, onupdate=utcnow)
...
class RuntimeControl(Base):
    ...
    current_interval_seconds: Mapped[int] = mapped_column(Integer, default=0)
    status_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    last_control_action: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    last_control_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
